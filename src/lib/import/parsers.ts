...
export function parseOFX(text: string): { rows: RawTx[]; banco?: string; cartao: boolean } {
  const blocks = text.split(/<STMTTRN>/i).slice(1).map((b) => b.split(/<\/STMTTRN>/i)[0]);
  if (!blocks.length) throw new ParseError("Nenhum lançamento encontrado no OFX");
  const rows: RawTx[] = [];
  for (const b of blocks) {
    ...
    const desc = name && memo && !memo.toUpperCase().includes(name.toUpperCase()) ? `${name} ${memo}` : memo || name || "";
    const valor = parseFloat(amt.replace(",", "."));
    if (!isFinite(valor)) continue;
    rows.push({ data: `${dt.slice(0, 4)}-${dt.slice(4, 6)}-${dt.slice(6, 8)}`, descricaoOriginal: desc.replace(/\s+/g, " ").trim(), valor });
  }
...
function parseDate(s: string, fallbackYear: number): ISODate | null {
  ...
  if (m) {
    const y = m[3] ? (m[3].length === 2 ? 2000 + Number(m[3]) : Number(m[3])) : fallbackYear;
    return `${y}-${m[2].padStart(2, "0")}-${m[1].padStart(2, "0")}`;
  }
  return null;
}

export interface CSVResult { rows: RawTx[]; formato: "nubank-fatura" | "generico" }

export function parseCSV(text: string, fallbackYear = new Date().getFullYear()): CSVResult {
  ...
  const iDesc = find("descricao", "title", "historico", "lancamento", "estabelecimento", "description", "memo");
  ...
  if (iData < 0 || iDesc < 0 || (iValor < 0 && iDeb < 0)) {
    throw new ParseError("Não reconheci as colunas do CSV (preciso de data, descrição e valor)");
  }
  ...
  for (const line of lines.slice(1)) {
    ...
    else valor = (parseValorBR(c[iCred] || "0") || 0) - Math.abs(parseValorBR(c[iDeb] || "0") || 0);
    if (!isFinite(valor) || valor === 0) continue;
    if (nubank) valor = -valor;
    rows.push({ data, descricaoOriginal: (c[iDesc] || "").replace(/\s+/g, " "), valor });
  }
...
export function parsePDFText(lines: string[], opts: { cartao: boolean; fallbackYear: number }): RawTx[] {
  const rows: RawTx[] = [];
  const re = /^(\d{2}[/.]\d{2}(?:[/.]\d{2,4})?)\s+(.+?)\s+(-|−)?\s?(?:R\$\s?)?(\d{1,3}(?:\.\d{3})*,\d{2})\s*([DC-])?$/i;
