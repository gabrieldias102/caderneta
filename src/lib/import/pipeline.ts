...
const MODELO: [RegExp, string, number][] = [
  ...
  [
    /MERCADO\s*LIVRE|MERCADOLIVRE|AMAZON|SHOPEE|MAGALU|MAGAZINE|AMERICANAS|SHEIN|ALIEXPRESS|RENNER|C&A|ZARA/,
    "compras",
    0.72,
  ],
  [
    /PAO\s*DE\s*ACUCAR|CARREFOUR|ASSAI|ATACADAO|\bEXTRA\b|\bDIA\b|SUPERMERCADO|MERCADO\b|HORTIFRUTI|SACOLAO|FEIRA|ACOUGUE/,
    "mercado",
    0.9,
  ],
  [
    /ALUGUEL|IMOBILIARIA|CONDOMINIO|ENEL|SABESP|COMGAS|\bLIGHT\b|CEMIG|VIVO|INTERNET|IPTU/,
    "moradia",
    0.9,
  ],
  [
    /CINEMA|CINEMARK|INGRESSO|SYMPLA|TEATRO|\bSHOW\b|STEAM|PLAYSTATION|XBOX/,
    "lazer",
    0.88,
  ],
  ...
  [
    /BAR\b|BOTECO|CHOPERIA|PUB\b|RESTAURANTE|LANCHONETE|PIZZARIA|HAMBURGUERIA/,
    "lazer",
    0.44,
  ],
...
export function sugerirCategoria(
  ...
  ctx: Pick<Contexto, "regras" | "lancamentos">,
) {
  const k = chave(estabelecimento);
  const regra = ctx.regras.find((r) => chave(r.estabelecimento) === k);
  if (regra)
    return { categoriaId: regra.categoriaId, confianca: 1, porRegra: true };

  const hist = ctx.lancamentos.filter(
    (t) => t.categoriaId && !t.tipo && chave(t.estabelecimento) === k,
  );
  if (hist.length >= 2) {
    const cont: Record<string, number> = {};
    hist.forEach(
      (t) => (cont[t.categoriaId!] = (cont[t.categoriaId!] || 0) + 1),
    );
    const [cat, n] = Object.entries(cont).sort((a, b) => b[1] - a[1])[0];
    return {
      categoriaId: cat,
      confianca: Math.min(0.96, 0.7 + 0.26 * (n / hist.length)),
      porRegra: false,
    };
  }

  const texto = strip(`${descricaoOriginal} ${estabelecimento}`);
  for (const [re, cat, conf] of MODELO)
    if (re.test(texto))
      return { categoriaId: cat, confianca: conf, porRegra: false };
...
export function acharDuplicata(
  item: { data: string; valor: number; estabelecimento: string; tipo?: string },
  contaId: string,
  lancamentos: Lancamento[],
) {
  const k = chave(item.estabelecimento);
  const min = addDays(item.data, -1),
    max = addDays(item.data, 1);
...
export function montarRevisao(rows: RawTx[], ctx: Contexto): ItemRevisao[] {
  return rows.map((r, i) => {
    ...
    const sug =
      neutro || pixPF
        ? { categoriaId: "", confianca: neutro ? 1 : 0, porRegra: false }
        : sugerirCategoria(det.estabelecimento, r.descricaoOriginal, ctx);
    // Pix para PF: só a regra do próprio usuário decide.
    if (pixPF) {
      const regra = ctx.regras.find(
        (x) => chave(x.estabelecimento) === chave(det.estabelecimento),
      );
      if (regra)
        Object.assign(sug, {
...
export function precisaRevisao(r: ItemRevisao) {
  ...
  return (
    ...
    n === "nenhuma" ||
    !r.categoriaId ||
    (!!r.pix?.pessoaFisica && !r.editado && !r.porRegra)
  );
}

/** Tenta identificar o banco/conta pelo nome do arquivo ou pelo conteúdo. */
export function detectarDestino(
  nomeArquivo: string,
  conteudo: string,
  contas: Conta[],
  cartao?: boolean,
...
export function ajustarSinal(rows: RawTx[], conta: Conta): RawTx[] {
  ...
  return positivos / rows.length > 0.6
    ? rows.map((r) => ({ ...r, valor: -r.valor }))
    : rows;
}
