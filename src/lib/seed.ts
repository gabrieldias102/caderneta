...
export const CONTAS: Conta[] = [
  ...
  },
  {
    ...
    tipo: "conta",
    banco: "itau itaú",
    sub: "Conta corrente · ag. 0421",
    saldo: 8905.4,
    ultimoExtrato: "20/09 (PDF)",
  },
  {
    id: "dinheiro",
    nome: "Dinheiro",
    ...
    saldo: 180,
    ultimoExtrato: "Atualizado em 15/09",
  },
  {
    id: "nucc",
    nome: "Nubank Ultravioleta",
    tipo: "cartao",
    banco: "nubank nu ultravioleta",
    sub: "Crédito •••• 4821",
    faturaAtual: 1842.37,
    limite: 12000,
...
const SPLIT: Record<string, [string, string, string, number][]> = {
  ...
  delivery: [
    ["iFood", "iFood", "nucc", 0.45],
    ["Rappi", "Rappi", "nucc", 0.25],
    ["iFood", "iFood", "nucc", 0.3],
  ],
  mercado: [
    ["Pão de Açúcar", "Pão de Açúcar", "itaucc", 0.55],
    ["Carrefour", "Carrefour", "itaucc", 0.35],
    ["Feira de domingo", "Feira", "dinheiro", 0.1],
  ],
  transporte: [
    ["Posto Shell", "Posto Shell", "itaucc", 0.6],
    ["Uber", "Uber", "nucc", 0.25],
    ["99", "99", "nucc", 0.15],
  ],
  lazer: [
    ["Cinemark", "Cinemark", "nucc", 0.4],
    ["Bar do Zé", "Bar do Zé", "nucc", 0.6],
  ],
};
...
function historico(): Lancamento[] {
  ...
  const add = (
    ...
    out.push({
      id: `h${n++}`,
      data,
      descricaoOriginal: descricao.toUpperCase(),
      descricao,
      estabelecimento: est,
  ...
  HIST.forEach((h, mi) => {
    ...
    if (h.r > 7800)
      add(
        d(18),
        "Pix recebido · Freela site",
        "Estúdio Ponto",
        h.r - 7800,
        "nuconta",
        "salario",
      );
    add(d(15), "Aluguel", "Imobiliária Lar", -2200, "itau", "moradia", {
      compartilhado: true,
    });
    add(
      d(10),
    ...
    for (const k of ["delivery", "mercado", "transporte", "lazer"] as const) {
      const total = h.cat[k];
      SPLIT[k].forEach(([desc, est, conta, f], i) =>
        add(d(3 + i * 8 + (mi % 3)), desc, est, -total * f, conta, k),
      );
      used += total;
    }
    ...
    if (h.ym >= "2026-05") {
      add(d(12), "Apple · iPhone 15", "Apple", -499.9, "itaucc", "compras", {
        parcela: { atual: mi, total: 12 },
      });
      used += 499.9;
    }
    if (h.ym >= "2026-07") {
    ...
    add(d(20), "Pagamento fatura Nubank", "", -2100 - mi * 57, "itau", "", {
      tipo: "fatura",
...
export function seedState(): DataState {
  const set: Lancamento[] = SETEMBRO.map(
    ([id, data, descricao, est, valor, contaId, cat, extra]) => ({
      id: String(id),
      data,
      descricaoOriginal: descricao.toUpperCase(),
      descricao,
  ...
  return {
    ...
    regras: [
      {
        ...
        origem: "Criada em 02/07 ao revisar importação",
      },
      {
        id: "r2",
    ...
    importacoes: [
      {
        id: "i1",
        arquivo: "fatura-nubank-ago-2026.pdf",
        contaId: "nucc",
        data: "2026-09-20",
        total: 47,
        ignorados: 0,
      ...
      {
        id: "i3",
        arquivo: "fatura-itau-click-ago.csv",
        contaId: "itaucc",
        data: "2026-09-12",
        total: 9,
        ignorados: 0,
