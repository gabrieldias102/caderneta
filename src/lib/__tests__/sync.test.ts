...
describe("diffState", () => {
  ...
  it("envia só o lançamento alterado", () => {
    const b = { ...a, lancamentos: a.lancamentos.map((t) => (t.id === "2" ? { ...t, categoriaId: "presentes" } : t)) };
    const d = diffState(a, b)!;
    expect(d.lancamentos?.upsert).toHaveLength(1);
    expect(d.lancamentos?.upsert?.[0]).toMatchObject({ id: "2", categoriaId: "presentes" });
    expect(Object.keys(d)).toEqual(["lancamentos"]);
  });

  it("detecta inclusão, exclusão e ordem de categorias", () => {
    const b = { ...a, categorias: [...a.categorias.slice(1), { id: "pets", nome: "Pets" }] };
    const d = diffState(a, b)!;
    expect(d.categorias?.delete).toEqual(["mercado"]);
    expect(d.categorias?.upsert?.find((c) => c.id === "pets")).toMatchObject({ ordem: b.categorias.length - 1 });
  });

  it("orçamento zerado vira exclusão", () => {
    const b = { ...a, orcamentos: { ...a.orcamentos, casa: 0, lazer: 450 } };
    expect(diffState(a, b)!.orcamentos).toEqual({ upsert: [{ categoriaId: "lazer", limite: 450 }], delete: ["casa"] });
  });

  it("perfil: prefs e remoção do grupo", () => {
    const b = { ...a, prefs: { ...a.prefs, thr: 90 }, grupo: undefined };
    expect(diffState(a, b)!.perfil).toEqual({ prefs: b.prefs, grupoRemovido: true });
  });
});
...
describe("mergePayload", () => {
  it("o mais novo vence e exclusão cancela upsert", () => {
    ...
    const m = mergePayload(
      { lancamentos: { upsert: [{ ...t1, descricao: "A" }] }, perfil: { nome: "X" } },
      { lancamentos: { upsert: [{ ...t1, descricao: "B" }] }, perfil: { prefs: seedState().prefs } },
    );
