...
import { ajustarSinal, montarRevisao, nivelConfianca, precisaRevisao } from "../import/pipeline";
...
describe("parseValorBR", () => {
  it.each([
    ["1.234,56", 1234.56], ["-58,90", -58.9], ["R$ 25", 25], ["(12,50)", -12.5], ["1234.56", 1234.56], ["1.000", 1000], ["85,00-", -85],
...
describe("detectar", () => {
  ...
  it("detecta parcelas", () => {
    ...
    expect(detectar("LOJA X PARC 3/10").parcela).toEqual({ atual: 3, total: 10 });
  });
  it("detecta Pix para pessoa física e para empresa", () => {
    ...
    expect(detectar("PIX RECEBIDO ESTUDIO PONTO LTDA").pix).toEqual({ pessoaFisica: false });
    expect(detectar("PIX TRANSF ***.123.456-** JOANA").pix?.pessoaFisica).toBe(true);
  });
...
describe("parsers", () => {
  it("lê OFX SGML", () => {
    ...
    expect(r.rows[0]).toEqual({ data: "2026-09-22", descricaoOriginal: "SABESP AGUA 09/26", valor: -89.9 });
  ...
  it("lê linhas de texto de PDF", () => {
    const rows = parsePDFText(["21/09 IFOOD *IFOOD 52,30", "Total da fatura 1.842,37", "08/09 PAGAMENTO RECEBIDO -2.340,50"], { cartao: true, fallbackYear: 2026 });
    expect(rows).toEqual([
      { data: "2026-09-21", descricaoOriginal: "IFOOD *IFOOD", valor: -52.3 },
      { data: "2026-09-08", descricaoOriginal: "PAGAMENTO RECEBIDO", valor: 2340.5 },
    ]);
  });
  it("inverte sinal de CSV genérico de cartão com compras positivas", () => {
    const s = seedState();
    const cartao = s.contas.find((c) => c.id === "itaucc")!;
    expect(ajustarSinal([{ data: "2026-09-01", descricaoOriginal: "X", valor: 10 }, { data: "2026-09-01", descricaoOriginal: "Y", valor: 5 }], cartao)[0].valor).toBe(-10);
  });
});

describe("montarRevisao (fatura Nubank de exemplo)", () => {
  const s = seedState();
  const rows = montarRevisao(parseCSV(exemplo("fatura-nubank-set.csv")).rows, { contaId: "nucc", regras: s.regras, lancamentos: s.lancamentos, contas: s.contas });
  const by = (m: string) => rows.find((r) => r.estabelecimento === m)!;

  it("aplica regras do usuário primeiro", () => {
    expect(by("iFood")).toMatchObject({ categoriaId: "delivery", porRegra: true });
    expect(nivelConfianca(by("Spotify"))).toBe("regra");
  });
  it("sugere Compras para Mercado Livre com confiança média", () => {
    expect(by("Mercado Livre")).toMatchObject({ categoriaId: "compras", parcela: { atual: 2, total: 6 } });
    expect(nivelConfianca(by("Mercado Livre"))).toBe("media");
  });
  it("marca duplicatas como ignoradas", () => {
    expect(by("Netflix")).toMatchObject({ duplicataDe: "18/09", ignorado: true });
  ...
  it("pede revisão para Pix PF e confiança baixa", () => {
    ...
    expect(by("Bar do Zé")).toMatchObject({ categoriaId: "lazer", porRegra: false });
...
describe("montarRevisao (extrato Itaú)", () => {
  const s = seedState();
  const rows = montarRevisao(parseOFX(exemplo("extrato-itau-set.ofx")).rows, { contaId: "itau", regras: s.regras, lancamentos: s.lancamentos, contas: s.contas });
  it("casa transferência entre contas próprias", () => {
    expect(rows.find((r) => r.descricaoOriginal.startsWith("TED RECEBIDA"))!.tipo).toBe("transferencia");
  });
  it("reconhece o pagamento de fatura já lançado como duplicata", () => {
    expect(rows.find((r) => r.tipo === "fatura")).toMatchObject({ duplicataDe: "20/09", ignorado: true });
  });
});
