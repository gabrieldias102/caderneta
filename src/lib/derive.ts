...
export const kindOf = (t: Lancamento): Kind =>
  t.tipo ? "neutral" : t.valor > 0 ? "income" : "expense";
export const isPixPendente = (t: Lancamento) =>
  !!t.pix?.pessoaFisica && !t.categoriaId && !t.tipo;

export const catNome = (s: DataState, id?: string) =>
  s.categorias.find((c) => c.id === id)?.nome ?? "Sem categoria";
export const contaNome = (s: DataState, id: string) =>
  s.contas.find((c) => c.id === id)?.nome ?? "—";
...
export function periodo(hoje: string) {
  ...
  return {
    ym,
    diasRestantes: Math.max(0, dias - dia),
    pctMes: Math.round((dia / dias) * 100),
  };
}

export function doMes(s: DataState, ym: string, ateHoje?: string) {
  return s.lancamentos.filter(
    (t) => monthOf(t.data) === ym && (!ateHoje || t.data <= ateHoje),
  );
}
...
export function totais(txs: Lancamento[]) {
  ...
  for (const t of txs) {
    ...
    if (k === "expense") {
      desp -= t.valor;
      if (t.categoriaId)
        porCat[t.categoriaId] = (porCat[t.categoriaId] || 0) - t.valor;
...
export function orcamentos(s: DataState, porCat: Record<string, number>) {
  ...
  const livre = entries.reduce(
    (a, [k, l]) => a + Math.max(0, l - (porCat[k] || 0)),
    0,
  );
...
export function parcelasAtivas(s: DataState, ym: string) {
  ...
  const itens = s.lancamentos.filter(
    (t) => t.parcela && kindOf(t) === "expense" && monthOf(t.data) === ym,
  );
  return itens.map((t) => ({
    t,
    restantes: t.parcela!.total - t.parcela!.atual,
    porMes: -t.valor,
  }));
}
