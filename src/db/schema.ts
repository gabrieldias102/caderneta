import {
  boolean, date, index, integer, jsonb, numeric, pgTable, primaryKey, text, timestamp, uuid,
...
const dinheiro = (name: string) => numeric(name, { precision: 14, scale: 2, mode: "number" });
...
const owned = { userId: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }), id: text("id").notNull() };
...
export const lancamentos = pgTable("lancamentos", {
  ...
  origem: text("origem", { enum: ["arquivo", "manual", "exemplo"] }).notNull(),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (t) => [
...
export const regras = pgTable("regras", {
  ...owned,
