...
const ops = <T extends z.ZodType>(item: T) =>
  z.object({ upsert: z.array(item).max(5000).optional(), delete: z.array(id).max(5000).optional() }).optional();
...
export const lancamentoSchema = z.object({
  ...
  parcela: z.object({ atual: z.number().int().min(1).max(99), total: z.number().int().min(1).max(99) }).optional(),
  compartilhado: z.boolean().optional(),
  importacaoId: id.optional(),
  origem: z.enum(["arquivo", "manual", "exemplo"]),
});

export const categoriaSchema = z.object({ id, nome: texto(60).min(1), ordem: z.number().int().optional() });
...
export const regraSchema = z.object({ id, estabelecimento: texto(120).min(1), categoriaId: id, origem: texto(200) });

export const importacaoSchema = z.object({
  id, arquivo: texto(260), contaId: id, data, total: z.number().int().min(0), ignorados: z.number().int().min(0),
});
...
export const grupoSchema = z.object({
  nome: texto(80).min(1),
  parceiro: z.object({ nome: texto(80).min(1), iniciais: texto(3), entrouEm: texto(20) }),
  split: z.number().int().min(0).max(100),
  acertadoEm: data.optional(),
  convites: z.array(texto(120)).max(50),
  gastosParceiro: z.array(z.object({ id, data, descricao: texto(), valor })).max(2000),
});
...
export const syncSchema = z.object({
  ...
  perfil: z.object({
    ...
    resumoIA: z.object({ mes: z.string().regex(/^\d{4}-\d{2}$/), texto: texto(2000) }).optional(),
  }).optional(),
});
