...
export async function lerJson<T extends z.ZodType>(req: Request, schema: T): Promise<{ ok: true; data: z.infer<T> } | { ok: false; res: NextResponse }> {
  const body = await req.json().catch(() => undefined);
  const r = schema.safeParse(body);
  if (!r.success) return { ok: false, res: erro(r.error.issues[0]?.message ?? "Dados inválidos", 400) };
...
export function autenticado(fn: Handler) {
  return async (req: Request) => {
    if (req.method !== "GET" && !mesmaOrigem(req)) return erro("Origem não permitida", 403);
