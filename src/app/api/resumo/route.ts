...
function isAgregados(b: unknown): b is Agregados {
  ...
  return (
    !!x &&
    typeof x.mes === "string" &&
    typeof x.receitas === "number" &&
    typeof x.despesas === "number" &&
...
export const POST = autenticado(async (req, user) => {
  ...
  if (!(await permitirTentativa(`resumo:${user.id}`, 20, 24 * 3600_000))) {
    return NextResponse.json(
      { erro: "Limite diário de resumos atingido — tente amanhã" },
      { status: 429 },
    );
  }
  ...
  const fatos = [
    ...
    ...body.categorias
      ...
      .map(
        (c) =>
          `- ${c.nome}: ${brl0(c.valor)} / ${brl0(c.mesAnterior)} / ${c.limite ? brl0(c.limite) : "sem limite"}`,
      ),
  ...
  try {
    ...
    if (msg.stop_reason === "refusal")
      return NextResponse.json(
        { erro: "Não foi possível gerar agora" },
        { status: 502 },
      );
    const texto = msg.content
      ...
      .trim();
    if (!texto)
      return NextResponse.json({ erro: "Resposta vazia" }, { status: 502 });
    return NextResponse.json({ texto });
  } catch (e) {
    if (e instanceof Anthropic.AuthenticationError)
      return NextResponse.json(
        { erro: "ANTHROPIC_API_KEY inválida" },
        { status: 503 },
      );
    if (e instanceof Anthropic.RateLimitError)
      return NextResponse.json(
        { erro: "Muitas solicitações — tente em instantes" },
        { status: 429 },
      );
    if (e instanceof Anthropic.APIError)
      return NextResponse.json(
        { erro: `Erro da API (${e.status})` },
        { status: 502 },
      );
